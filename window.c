...
// for non-sensitive selection
#include <stdarg.h>
static HANDLE hStdOut = NULL;
static HANDLE hLogFile = NULL;
static DWORD dbg_last = 0;

int OpenDebugConsole(DWORD odc_mode)
{
    if (!hStdOut) {
        if (odc_mode == 0) {
            AllocConsole();
...
void Debug(const char *fmt,...)
{
    ...
    va_list ap;
    va_start(ap, fmt);
    len += (DWORD)vsnprintf(buf + len, sizeof(buf) - len -1, fmt, ap);
    va_end(ap);
...
#define SELECT_TIMER_ID 2001
// 0:none, 1:first-click-and-checktimer\, 2:check-done-and-fire-click, 3:double/triple-click
int PseudoSelectMode;
DWORD  SelTimerId;
ULONGLONG LastSelectTick;
                                ...
                                WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message) {
      case WM_TIMER:
        ...
        // for non-sensitive selection
        if (wParam == SELECT_TIMER_ID) {
            // Debug("timer p=%d\n",  PseudoSelectMode);
            if (SelTimerId) {
                KillTimer(hwnd, SELECT_TIMER_ID);
                SelTimerId = 0;
            }
            POINT pt;
            GetCursorPos(&pt);
            if (PseudoSelectMode == 1) {
                if (abs(SelPt.x - pt.x) < 5 && abs(SelPt.y - pt.y) < 5) {
                    PseudoSelectMode = 2;
                    // Debug("timer to 2\n");
                    PostMessage(hwnd, WM_LBUTTONDOWN, WParamSv, LParamSv);
                } else {
                    PseudoSelectMode = 0;
                    // Debug("timer to 0\n");
                }
        ...
        return 0;
      case WM_ACTIVATEAPP: // for non-sensitive selection
...
#define TO_CHR_Y(y) ((((y)<0 ? (y)-font_height+1: (y))-offset_height) / font_height)
      ...
      case WM_RBUTTONUP:
        ...
        }
        {
            ...
            switch (message) {
              case WM_LBUTTONDOWN:
                cur = GetTickCount64(); // for non-sensitive selection
                // Debug("lb p=%d dbl=%d act=%lld\n",  PseudoSelectMode, dbltime, cur - LastActiveTick);
                button = MBT_LEFT;
                wParam |= MK_LBUTTON;
                press = true;
                if (SelTimerId) {
                ...
                if (cur - LastSelectTick > dbltime) {
                    PseudoSelectMode = 0;
                    LastSelectTick = 0;
                }
                if (PseudoSelectMode == 0) {
                    if (cur - LastActiveTick < dbltime) {
                        press = false; // nothing
                    }
                    else if (cur - LastSelectTick <= dbltime) {
                        // Debug("lb to 3\n");
                        PseudoSelectMode = 3;
                        LastSelectTick = cur;
                        ...
                        LastSelectTick = cur;
                        press = false;
                        SelTimerId = SetTimer(hwnd, SELECT_TIMER_ID, 100, 0);
                    }
                }
                else if (PseudoSelectMode == 1) {
                    // Debug("lb 1 to 0\n");
                    press = false;
                    PseudoSelectMode = 0;
                }
                else if (PseudoSelectMode == 2) {
                    // Debug("lb 2 to 0\n");
                    PseudoSelectMode = 0;
                    button = MBT_LEFT;
                    wParam |= MK_LBUTTON;
